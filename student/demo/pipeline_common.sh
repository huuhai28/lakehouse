...
common::update_trino_token() {
  ...
  docker exec polaris-proxy sh -c "sed -i 's/warehouse=[^\"]*/warehouse='${CATALOG_NAME}'/' /etc/nginx/conf.d/default.conf && nginx -s reload" > /dev/null 2>&1 || true
  docker cp trino-conf/iceberg.properties trino:/etc/trino/catalog/iceberg.properties && docker restart trino > /dev/null 2>&1
...
common::setup_polaris_grants() {
  ...
  local catalog_role="catalog_admin"
  local principal_role="service_admin"

  echo "  🔐 [Polaris] Setup grants: ${principal_role} → ${catalog_role} → ${catalog}..."

  # Bước 1: Tạo catalog-role (bỏ qua nếu đã tồn tại 409)
  R=$(curl -s -o /dev/null -w "%{http_code}" \
    -X POST "${base_url}/api/management/v1/catalogs/${catalog}/catalog-roles" \
    -H "Authorization: Bearer ${token}" \
    -H "Content-Type: application/json" \
    -d "{\"catalogRole\": {\"name\": \"${catalog_role}\"}}")
  echo "    [1] Tao catalog-role '${catalog_role}': HTTP ${R}"

  # Bước 2: Grant CATALOG_MANAGE_CONTENT
  R=$(curl -s -o /dev/null -w "%{http_code}" \
    -X PUT "${base_url}/api/management/v1/catalogs/${catalog}/catalog-roles/${catalog_role}/grants" \
    -H "Authorization: Bearer ${token}" \
    -H "Content-Type: application/json" \
    -d '{"grant":{"type":"catalog","privilege":"CATALOG_MANAGE_CONTENT"}}')
  echo "    [2] Grant CATALOG_MANAGE_CONTENT: HTTP ${R}"

  # Bước 3: Gắn catalog-role vào service_admin
  R=$(curl -s -o /dev/null -w "%{http_code}" \
    -X PUT "${base_url}/api/management/v1/principal-roles/${principal_role}/catalog-roles/${catalog}" \
    -H "Authorization: Bearer ${token}" \
    -H "Content-Type: application/json" \
    -d "{\"catalogRoleName\": \"${catalog_role}\"}")
  echo "    [3] Gan catalog-role vao ${principal_role}: HTTP ${R}"

  echo "  ✅ Grants da thiet lap."
}
